...
namespace Microsoft.TeamServices.Samples.Client.WorkItemTracking
{
	...
	public class WorkItemsSample : ClientSample
	{
		...
		{
			int[] workitemIds = new int[] { 693488 };
		...
		[ClientSampleMethod]
		public async Task<int> GetWorkItemById()
		{
			var resultCount = 0;
			...
			//var queries=workItemTrackingClient.GetQueriesAsync("OneCRM");
			Guid queryId = new Guid("46bfb5d9-476f-4927-91fc-f13ad3ccf76e");
			try
			{
				var workItemQueryResult = workItemTrackingClient.QueryByIdAsync(queryId).Result;
				var listOfIds = workItemQueryResult.WorkItems.Select(w => w.Id);
				var workItems = GetWorkItemsWithSpecificFields(listOfIds);
				string reprosteps = workItems[0].Fields.ElementAt(3).Value.ToString();
				resultCount = listOfIds.Count();
				int pullRequestId = 79900;
				var userCommitRef = GetCommitIdForPRMaster(pullRequestId);
				var changes=GetChanges(userCommitRef.CommitId.ToString());
				var fileName=changes.Changes.ElementAt(0).Item.Path;
		...
		public GitCommitChanges GetChanges(string commitID)
		{
			Guid repositoryId = new Guid("b0f0d085-2822-4f4e-98e6-e8b50e7c11c2");
			VssConnection connection = this.Context.Connection;
			GitHttpClient gitClient = connection.GetClient<GitHttpClient>();
			var changes= gitClient.GetChangesAsync(commitID, repositoryId).Result;
			return changes;
		}

		[ClientSampleMethod]
		public GitCommitRef GetCommitIdForPRMaster(int pullRequestId)
		{
			Guid repositoryId = new Guid("b0f0d085-2822-4f4e-98e6-e8b50e7c11c2");
			VssConnection connection = this.Context.Connection;
			GitHttpClient gitClient = connection.GetClient<GitHttpClient>();
			
			var pr = gitClient.GetPullRequestAsync(repositoryId, pullRequestId).Result;
			if (pr == null)
				throw new Exception(String.Format("PR {0} not found", pullRequestId));
			var userCommit = pr.LastMergeCommit;
			var masterLastCommit = pr.LastMergeTargetCommit;
			return userCommit;
		}


		[ClientSampleMethod]
		public List<WorkItem> GetWorkItemsWithSpecificFields(IEnumerable<int> workitemIds = null)
		{
			int[] arr = new int[] { 405962 };
			string[] fieldNames = new string[] {
			   "System.Id",
			   "System.Title",
			   "System.AreaPath",
			...
			VssConnection connection = Context.Connection;
			WorkItemTrackingHttpClient workItemTrackingClient = connection.GetClient<WorkItemTrackingHttpClient>();
			List<WorkItem> workitems = workItemTrackingClient.GetWorkItemsAsync(arr, fieldNames).Result;
			return workitems;
		}


		[ClientSampleMethod]
		public WorkItem GetWorkItemByIdTemp()
		{
			int id = 693488;
